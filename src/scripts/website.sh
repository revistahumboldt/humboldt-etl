...
RUN_JOB_NAME="website-daily"
SCHEDULER_JOB_NAME="website-daily"
...
if [ "$RUN_JOB_NAME" ]; then
  ...
  if gcloud run jobs describe "$RUN_JOB_NAME" --region "$GCP_REGION" &> /dev/null; then
    ...
    gcloud run jobs update "$RUN_JOB_NAME" \
      --image "$FULL_IMAGE" \
      --region "$GCP_REGION" \
      --env-vars-file=src/config/env-vars-website.yaml
...
# 2. Create or update Cloud Scheduler Jobs
...
if gcloud scheduler jobs describe "$RUN_JOB_NAME" --location="$GCP_REGION" &> /dev/null; then
  ...
  gcloud scheduler jobs update http "$RUN_JOB_NAME" \
    --location="$GCP_REGION" \
    --schedule="$SCHEDULE" \
    --http-method=POST \
  ...
  gcloud scheduler jobs create http "$RUN_JOB_NAME" \
    --location="$GCP_REGION" \
    --schedule="$SCHEDULE" \
    --http-method=POST \
    --uri="$FAD_SCHEDULER_URI" \
...
# 3. Manually trigger Cloud Run jobs
